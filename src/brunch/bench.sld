...
(begin
  ...
  (define (bench-run b proc)
    (cond 
      ...
      [else 
        ...
        (define samples (bench-samples b))
        (let loop ([times '()] [i 0])
          (cond 
            [(< i samples)
              (define now2 (current-second))
  ...
  (define (benches-finish benches)
    ...
    (let loop ([summary (list #f
                          #("\x1b;[1;95mMethod" "Mean" "Samples\x1b;[0m" "\x1b;[1;95mChange\x1b;[0m"))] [bs (benches-benches benches)])
        ...
        [else 
          ...
          (cond 
            [(bench-spacer? b)  
              (loop (cdr bs))]
            [else 
              (define time (number->string (stats-mean (bench-stats b))))
              (let-values ([(valid total) (stats-samples (bench-stats b))])
  ...
  (define (format-summary table port)
    (let-values ([(w1 w2 w3 w4) (lens table)])
      ...
      (define pad-len (max w1 w2 w3 w4))
      (define pad (make-string pad-len #\space))
      (define spacer-p (open-output-string))
      ...
      (let loop ([i 0])
        (when (< i width)
          ...
          (loop (+ i 1))))
      (write-string "\x1b;[0m\n" spacer-p)
      (define spacer (get-output-string spacer-p))
      ...
      (let loop ([rows table])
        (cond 
          ...
          [else 
            (define row (car rows))
            (define v1 (vector-ref row 0))
            (define v2 (vector-ref row 1))
            (define v3 (vector-ref row 2))
            (define v4 (vector-ref row 3))
            (define c1 (get-width v1))
            (define c2 (get-width v2))
            (define c3 (get-width v3))
            (define c4 (get-width v4))
  ...
  (define (lens table)
    (let loop ([w1 0] [w2 0] [w3 0] [w4 0] [rows table])
      ...
      (cond 
        ...
        [else 
          (define row (car rows))
          (define v1 (get-width (vector-ref row 0)))
          (define v2 (get-width (vector-ref row 1)))
          (define v3 (get-width (vector-ref row 2)))
          (define v4 (get-width (vector-ref row 3)))
